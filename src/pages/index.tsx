...
const Home = () => {
  ...
  const download = async (type: string) => {
    ...
    try {
      ...
      const response = await fetch(`/api/download?url=${url}&type=${type}`, {
        method: "POST",
        headers: {
      ...
      if (response.ok) {
        ...
        setError(
          "Oops something went wrong with the download, please try again later!"
        );
      }
    } catch (error) {
      console.log("error:", error);
  ...
  return (
    <div className="mt-[10%]">
      <div className="flex items-center justify-center flex-col my-auto gap-6">
        ...
        <div role="status" hidden aria-label="spinner">
          <svg
            aria-hidden="true"
            className="w-8 h-8 text-gray-200 animate-spin dark:text-gray-600 fill-[#FF0101]"
            viewBox="0 0 100 101"
            fill="none"
